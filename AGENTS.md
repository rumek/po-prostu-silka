...
- Going up to a parent is `Up.to(parent)` (`core/layout/up.ts`), or `<a [appUp]>` in a template.
  It pops when the parent is below in history. Never write a plain `routerLink` back to a parent.
