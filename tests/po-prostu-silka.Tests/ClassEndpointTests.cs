...
public class ClassEndpointTests(IntegrationTestFixture fixture)
{
    ...
    private async Task<string> CreateAccountAsync(
        HttpClient admin, AccountStatus status, string role)
    {
    ...
    private Task<string> CreateTrainerAsync(HttpClient admin) =>
        CreateAccountAsync(admin, AccountStatus.Active, ApplicationRoles.Trainer);
    ...
    public async Task Edit_can_reassign_the_instructor()
    {
        ...
        var created = await PostClassAsync(admin, type.Id, slot, trainerId);
        var replacement = await CreateTrainerAsync(admin);
        ...
        Assert.Equal(
            replacement,
            (await response.Content.ReadFromJsonAsync<ClassBody>())!.InstructorUserId);
    }
