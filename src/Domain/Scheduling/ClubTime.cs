...
///
/// <para>
/// WHAT THIS CLASS DOES NOT DO IS FORMAT. It answers where the club is and what an instant reads as
/// there; the culture that names the month and the order the parts come in are presentation, and
/// they live with the renderer in <c>Application/Notifications/MessageTime</c>. Keeping the split
/// is what stops this file from becoming a template engine as more messages land.
/// </para>
///
/// <para>
...
public static class ClubTime
{
    ...
    /// <summary>
    /// The club's wall-clock reading of an instant (S-09) — the hour a member turns up for, as
    /// opposed to the UTC instant every JSON path still returns.
    ///
    /// <para>
    /// CONVERSION ONLY. How that reading is FORMATTED — which culture names the month, which order
    /// the parts come in — is a presentation decision and belongs to whoever renders the message,
    /// not to the Domain. See <c>Application/Notifications/MessageTime</c>.
    /// </para>
    /// </summary>
    public static DateTimeOffset ToClubLocal(DateTimeOffset instant) =>
